...
)  # This import loads all the models. May take a while
...
def inject_saver(
    ...
    for n, codeline in enumerate(code):
        ...
        if (
            codeline.startswith("#") or codeline == ""
        ):  # this will cause issues if you have lots of comment lines
            continue
        if "#" in codeline:
        ...
        if code_type in ("assign", "append", "if", "return", "for", "sort", "add"):
            ...
            if show_intermediate_steps:
                ...
                injection_string_format = (
                    lambda thing: f"{indent}CodexAtLine({n},syntax=syntax,"
            ...
            if isinstance(thing_to_show, list):
                injection_string_list = [
                    injection_string_format(f"{thing}") for thing in thing_to_show
                ]
...
def get_thing_to_show_codetype(codeline):
    ...
    if isinstance(thing_to_show, list):
        thing_to_show = [
            thing
            if not (thing.strip().startswith("'") and thing.strip().endswith("'"))
            else thing.replace("'", '"')
        ...
        thing_to_show = (
            thing_to_show
            if not (
                thing_to_show.strip().startswith("'")
                and thing_to_show.strip().endswith("'")
            )
            else thing_to_show.replace("'", '"')
        )
...
def CodexAtLine(lineno, syntax, time_wait_between_lines=1.0):
...
def get_code(query):
    ...
    if config.codex.model not in ("gpt-3.5-turbo", "gpt-4"):
        code = (
            f"def execute_command(image, my_fig, time_wait_between_lines, syntax):"
            + code
    ...
    syntax_1 = Syntax(
        code_for_syntax, "python", theme="monokai", line_numbers=True, start_line=0
    )
    ...
    syntax_2 = Syntax(
        code_for_syntax_2, "python", theme="monokai", line_numbers=True, start_line=0
    )
...
def execute_code(code, im, show_intermediate_steps=True):
    code, syntax = code
    code_line = inject_saver(
        code, show_intermediate_steps, syntax, time_wait_between_lines, console
    )
    ...
    with Live(
        Padding(syntax, 1), refresh_per_second=10, console=console, auto_refresh=True
    ) as live:
        my_fig = plt.figure(figsize=(4, 4))
        try:
            exec(compile(code_line, "Codex", "exec"), globals())
            result = execute_command(
                im, my_fig, time_wait_between_lines, syntax
            )  # The code is created in the exec()
...
def show_single_image(im, saved: bool = False):
    im = Image.fromarray(
